...
func createPostHandler() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		filename := fmt.Sprintf("%d-%s", time.Now().Unix(), payload.Logo.Filename)
		filepath := "/uploads/" + filename

		if err := ctx.SaveUploadedFile(payload.Logo, "."+filepath); err != nil {
			ctx.JSON(http.StatusInternalServerError, gin.H{
				"status":  "error",
				"message": "failed to save file to the server.",
			})
			return
		}
